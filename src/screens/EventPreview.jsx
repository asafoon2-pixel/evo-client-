    ...
    <div className="w-full min-h-screen bg-evo-black overflow-y-auto pb-20">
      ...
      <div className="relative h-80 bg-evo-card overflow-hidden">
        ...
        <motion.div {...f(0.3)} className="absolute bottom-8 left-0 right-0 text-center px-8">
          <p className="text-[10px] tracking-[0.35em] uppercase text-evo-accent mb-2 font-medium">Your Evening</p>
      ...
      <div className="px-6 space-y-10 pt-8">
        ...
        <motion.button {...f(0.1)}
          ...
          whileTap={{ scale: 0.98 }}
          className="w-full bg-evo-card rounded-2xl border border-evo-border p-5 flex items-center gap-5 hover:border-evo-accent/30 transition-all"
        >
          <motion.div
            animate={{ scale: [1, 1.08, 1] }}
            transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
            className="w-12 h-12 rounded-full border border-evo-accent flex items-center justify-center shrink-0"
          >
        ...
        {/* Atmosphere collage */}
        <motion.div {...f(0.15)}>
          <p className="text-[10px] tracking-[0.3em] uppercase text-evo-muted mb-4">{previewSections[0].label}</p>
          <div className="grid grid-cols-3 gap-2 mb-3">
            {previewSections[0].images.map((img, i) => (
              <div key={i} className={`rounded-xl overflow-hidden bg-evo-card ${i === 0 ? 'col-span-2' : ''}`} style={{ height: 140 }}>
                <img src={img} alt="" className="w-full h-full object-cover" />
              </div>
            ))}
          </div>
          <p className="text-white text-lg font-light tracking-wide">{previewSections[0].descriptor}</p>
        </motion.div>

        {/* Venue / Dining / Entertainment sections */}
        {previewSections.slice(1).map((section, i) => (
          <motion.div key={section.id} {...f(0.2 + i * 0.07)}>
            <p className="text-[10px] tracking-[0.3em] uppercase text-evo-muted mb-4">{section.label}</p>
            ...
            <div className="mt-4">
              <p className="text-white text-lg font-light">{section.descriptor}</p>
              <p className="text-evo-muted text-sm mt-2 font-light leading-relaxed">{section.detail}</p>
            </div>
          </motion.div>
        ))}
        ...
        {/* Color palette */}
        <motion.div {...f(0.4)}>
          <p className="text-[10px] tracking-[0.3em] uppercase text-evo-muted mb-4">PALETTE</p>
          <div className="flex gap-3 items-end">
            {palette.map((p, i) => (
              <div key={i} className="flex flex-col items-center gap-2 flex-1">
                <motion.div
                  className="w-full rounded-xl border border-white/5"
                  style={{ height: 48 + i * 8, backgroundColor: p.color }}
        ...
        <motion.div {...f(0.45)} className="text-center pt-2">
          <p className="text-evo-muted text-sm font-light mb-6 leading-relaxed">Your event is taking shape.</p>
          <motion.button onClick={() => navigate('management')} whileTap={{ scale: 0.97 }}
            className="w-full py-4 rounded-full bg-evo-accent text-black text-sm font-semibold tracking-[0.12em] uppercase shadow-[0_4px_24px_rgba(201,169,110,0.25)] hover:bg-[#B8946A] transition-all"
          >
            Back to My Event
          </motion.button>
        </motion.div>
        ...
        {showFilmModal && (
          ...
          >
            <button onClick={() => setShowFilmModal(false)}
              className="absolute top-12 right-6 w-10 h-10 rounded-full border border-white/20 flex items-center justify-center hover:border-white/50 transition-colors">
              <X size={18} className="text-white" />
            </button>

            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2 }} className="text-center max-w-xs w-full">
              <p className="text-[10px] tracking-[0.35em] uppercase text-evo-accent mb-8 font-medium">
              ...
              <div className="space-y-2 mb-10">
                {[
                  ...
                  <motion.div key={i}
                    ...
                    className="w-full rounded-xl overflow-hidden" style={{ height: 80 }}
                  >
                    <img src={img} alt="" className="w-full h-full object-cover" />
                  </motion.div>
                ))}
