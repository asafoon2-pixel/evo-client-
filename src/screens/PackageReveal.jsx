...
function TuneSheet({ onClose }) {
  ...
  const handleApply = () => {
    onClose()
    // In a real product this would re-run the build with bias. For now, go back to rebuild.
    navigate('building')
  }
        ...
        <div className="grid grid-cols-2 gap-2 mb-6">
          {TUNE_OPTIONS.map(o => (
            <button key={o.id} onClick={() => setSelected(o.id)}
              className={`py-3 px-4 rounded-xl border text-sm font-light transition-all ${selected === o.id ? 'border-evo-accent text-evo-accent bg-evo-accent/5' : 'border-evo-border text-evo-muted hover:text-white hover:border-evo-border/70'}`}>
              {o.label}
            </button>
          ))}
        </div>
        <button onClick={handleApply} disabled={!selected}
          className="w-full py-3.5 rounded-full border border-evo-accent text-evo-accent text-sm font-medium tracking-wider uppercase hover:bg-evo-accent hover:text-black transition-all disabled:opacity-30 disabled:cursor-not-allowed">
          Apply &amp; Rebuild
        </button>
...
export default function PackageReveal() {
  const { navigate, eventPackage, openSwapSheet, swapSheet, totalPrice, depositAmount } = useApp()
  ...
  return (
    <div className="w-full min-h-screen bg-evo-black overflow-y-auto pb-32">
      ...
      <div className="relative overflow-hidden" style={{ height: '70vw', maxHeight: 380, minHeight: 260 }}>
        <img src={eventPackage.heroImage} alt="" className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-black/10 to-evo-black" />
        ...
        <div className="absolute bottom-6 left-0 right-0 text-center px-8">
          <p className="text-xs tracking-[0.3em] uppercase text-evo-accent mb-2">EVO Built This For You</p>
          <h1 className="text-2xl sm:text-3xl font-light text-white leading-tight">{eventPackage.name}</h1>
        </div>
      </div>

      {/* Tags */}
      <div className="flex gap-2 justify-center px-6 mt-5 mb-8 flex-wrap">
        {eventPackage.tags.map(tag => (
          <span key={tag} className="text-xs font-medium tracking-widest uppercase text-evo-muted border border-evo-border rounded-full px-4 py-1.5">
            {tag}
          </span>
        ))}
      </div>
      ...
      {/* Description */}
      <p className="text-evo-muted text-sm font-light leading-relaxed text-center px-8 mb-10">
          ...
          >
            ...
            <div className="relative overflow-hidden mx-6 rounded-2xl mb-5" style={{ height: 220 }}>
              ...
              <div className="absolute top-4 left-4">
                <span className="text-xs font-medium tracking-[0.2em] uppercase bg-black/60 backdrop-blur-sm text-evo-muted border border-evo-border px-3 py-1.5 rounded-full">
                  {section.label}
                </span>
              </div>
            </div>
            ...
            <div className="px-6 mb-4">
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  ...
                  <p className="text-evo-muted text-sm mt-2 leading-relaxed font-light">
                    {section.vendor.description}
                  </p>
                  <div className="flex items-center gap-3 mt-3">
                    <span className="text-evo-accent text-sm font-medium">{formatPrice(section.vendor.price)}</span>
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-evo-dim text-xs tracking-wide hover:text-evo-muted transition-colors mt-1 shrink-0 border border-evo-border rounded-full px-3 py-1.5"
                >
                  Swap
            ...
            {section.constraint && (
              <motion.div
                ...
                className="mx-6 mb-4 bg-evo-elevated border border-evo-border rounded-2xl p-4 flex gap-3 items-start"
              >
                <AlertCircle size={14} className="text-evo-muted shrink-0 mt-0.5" />
                <div>
                  <p className="text-xs tracking-widest uppercase text-evo-muted mb-1">EVO Note</p>
      ...
      >
        ...
        {eventPackage.sections.map(s => (
          <div key={s.id} className="flex justify-between py-2 border-b border-evo-border/50 last:border-0">
        ...
        <div className="flex justify-between mt-4 pt-2">
          <span className="text-white text-sm">Total Event Value</span>
        ...
        <div className="flex justify-between mt-2">
          ...
          <span className="text-evo-accent text-sm font-medium">{formatPrice(depositAmount)}</span>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-evo-black/95 backdrop-blur-md border-t border-evo-border px-6 py-4 z-30">
        <button
          ...
          className="w-full py-4 rounded-full bg-evo-accent text-black text-sm font-medium tracking-wider uppercase hover:bg-evo-accent/90 transition-all active:scale-[0.98] mb-3"
        >
          Secure This Event
        </button>
        <button
